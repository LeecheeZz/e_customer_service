...
def main(argv=None):
    parser = argparse.ArgumentParser(description="Evaluate an SFT adapter")
    ...
    parser.add_argument("--out-file", default="sft_eval_outputs.jsonl")
    ...
    model_dir = args.model_dir or str(paths["sft_final_adapter_dir"])
    out_path = resolve_output_path(args.out_file, paths["sft_eval_dir"])
    ...
    with open(out_path, "w", encoding="utf-8") as out_f:
        for item in tqdm(list(read_jsonl(args.val_file)), desc="SFT Eval"):
            try:
    ...
    print("SFT eval saved to", out_path)
